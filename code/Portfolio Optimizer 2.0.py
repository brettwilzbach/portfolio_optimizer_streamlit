...
def load_roa_master():
    ...
    try:
        ...
        if file_path is None:
            raise FileNotFoundError(f"RoA Master Sheet not found in any of the expected locations")
            
        roa_master = pd.read_excel(file_path)
        roa_master.columns = [col.strip() for col in roa_master.columns]
        st.sidebar.success("✅ RoA Master Sheet loaded successfully from: " + os.path.basename(file_path))
        return roa_master
