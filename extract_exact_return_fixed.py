...
def extract_exact_return():
    ...
    # Convert Month column to datetime - handle errors explicitly
    try:
        ...
        print("Will use row indices for filtering instead of dates.")
        # Create a dummy Month column with sequential dates
        monthly_data['Month'] = pd.date_range(start='2020-01-01', periods=len(monthly_data), freq='M')
    ...
    for period_name, months in periods.items():
        ...
        if months is None:
            ...
            # Filter by row count if date filtering fails
            if pd.isna(monthly_data['Month']).any():
                # Just take the last N rows
                filtered_data = monthly_data.iloc[-months:].copy()
                print(f"Using last {months} rows of data (date filtering not possible)")
            else:
                ...
                # Filter data
                filtered_data = monthly_data[monthly_data['Month'] >= cutoff_date].copy()
                print(f"Using data from {cutoff_date} to {most_recent_date}")
        
        print(f"Using {len(filtered_data)} months of data")
        ...
        for col in available_strategies:
            if monthly_returns[col].dtype == 'object':
                ...
                try:
                    # Check if values contain '%' character
    ...
    monthly_returns_all = monthly_data[available_strategies].copy()
        ...
        for period_name, months in periods.items():
            ...
            if months is None:
                ...
                if pd.isna(monthly_data['Month']).any():
                    # Just take the last N rows
                    filtered_data = monthly_data.iloc[-months:].copy()
                    f.write(f"Using last {months} rows of data\n")
                else:
                    # Get the most recent date
                    most_recent_date = monthly_data['Month'].max()
                    # Calculate cutoff date
                    ...
                    # Filter data
                    filtered_data = monthly_data[monthly_data['Month'] >= cutoff_date].copy()
                    f.write(f"Using data from {cutoff_date} to {most_recent_date}\n")
